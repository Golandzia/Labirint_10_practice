...
namespace Labirint_10
{
    internal class Program
    {
        public delegate void BorderEventHandler(int x, int y);   //объявление делегата
        public static event BorderEventHandler OutOfBorder;
        static void Main(string[] args)
        {
            Program Action = new Program();
            OutOfBorder += Reaction;
        ...
        private static void Reaction(int x, int y) //реакция на событие источника
        {
            MessageBox.Show("Вы вышли из Лабиринта!\nПоздравляю!", "Поздравляю!", MessageBoxButton.OK, MessageBoxImage.Information);
        }
    }
}
